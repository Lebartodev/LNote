...
class ShowNoteFragment : BaseFragment() {
    ...
    private val viewModel: ShowNoteViewModel by viewModels { viewModelFactory }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.transitionName = resources.getString(
            R.string.note_container_transition_name,
            id?.toString() ?: "local"
        )
        ...
        binding.photosList.transitionName = resources.getString(
            R.string.note_photos_transition_name,
            id?.toString() ?: "local"
        )
        binding.backButton.setOnClickListener { parentFragmentManager.popBackStack() }

        val visibleTitleLimit = 56f.toPx(resources)
        binding.noteContent.setOnScrollChangeListener { _: NestedScrollView?, _: Int, scrollY: Int,
    ...
    private fun setupViewModel() {
        viewModel.note().observe(viewLifecycleOwner) { note ->
            note.run {
                ...
                binding.photosList.visibility =
                    if (note.photos.isEmpty()) View.GONE else View.VISIBLE
            }
        }
