...
class NotesAdapter(private val listener: (note: Note, sharedViews: List<View>) -> Unit) :
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        private val photosChip: Chip = itemView.findViewById(R.id.photos_chip)
        ...
        fun bind(item: Note, listener: (note: Note, sharedViews: List<View>) -> Unit) {
            noteContent.transitionName = itemView.resources.getString(
                    R.string.note_container_transition_name, item.id.toString())
            title.transitionName = itemView.resources.getString(R.string.note_title_transition_name,
                    item.id.toString())
            description.transitionName = itemView.resources.getString(
                    R.string.note_description_transition_name, item.id.toString())
            dateChip.transitionName = itemView.resources.getString(
                    R.string.note_date_transition_name, item.id.toString())
            ...
            photosChip.visibility = if (item.photos.isEmpty()) View.GONE else View.VISIBLE
        }
    }
